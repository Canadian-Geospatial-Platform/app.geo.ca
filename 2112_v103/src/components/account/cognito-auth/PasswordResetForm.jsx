...
class PasswordResetForm extends React.Component {
  ...
  render = () => (
    <div>
      ...
      <div className="pageMessage">{this.state.message}</div>
      <div className="authForm">
        <label>
          <span>Username</span>
          <input type="text" placeholder="username" value={this.state.username} onChange={this.changeUsername} required />
        </label>
        <button className="btn-button" type="button" onClick={this.sendVerificationCode}>Send verification code</button>
      </div>
      <div className="authForm">
        <label>
